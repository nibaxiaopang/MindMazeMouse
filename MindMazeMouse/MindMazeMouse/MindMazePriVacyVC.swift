...
class MindMazePriVacyVC: UIViewController , WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate{
    ...
    var privacyData: [Any]?
    @objc var url: String?
    let privacyPolicyUrl = "https://www.termsfeed.com/live/228ed3e5-410b-4cc7-b4e8-23f2ce7253c7"
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.privacyData = UserDefaults.standard.array(forKey: UIViewController.mindMazeGetUserDefaultKey())
        initSubViews()
        initNavView()
        initWebView()
        mindMazeStartLoadWebView()
    }
    ...
    private func mindMazeStartLoadWebView() {
    ...
    private func mindMazeReloadWebViewData(_ adurl: String) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            if let storyboard = self.storyboard,
               let adView = storyboard.instantiateViewController(withIdentifier: "MindMazePriVacyVC") as? MindMazePriVacyVC {
                adView.url = adurl
           ...
           let trackMessage = message.body as? [String: Any] {
            ...
            if let ty = confData[18] as? Int, ty == 1 {
                    ...
                    do {
                        if let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                            if tName != (confData[8] as? String) {
                                mindMazeSendEvent(tName, values: jsonObject)
                                return
                            }
                            ...
                            if let adId = jsonObject["url"] as? String, !adId.isEmpty {
                                mindMazeReloadWebViewData(adId)
                            }
                        }
                    } catch {
                        mindMazeSendEvent(tName, values: [tName: data])
                    }
                } else {
                    mindMazeSendEvent(tName, values: [tName: tData])
                }
            } else if let ty = confData[18] as? Int, ty == 2 {
                mindMazeAfSendEvents(tName, paramsStr: tData)
            } else {
                if tName == confData[28] as? String {
                    ...
                    mindMazeAfSendEvent(withName: tName, value: tData)
                }
            }
            
        } else if name == (confData[19] as? String) {
            if let messageBody = message.body as? String,
               let dic = mindMazeJsonToDic(withJsonString: messageBody) as? [String: Any],
               let evName = dic["funcName"] as? String,
               let evParams = dic["params"] as? String {
                
                if evName == (confData[20] as? String) {
                    if let uDic = mindMazeJsonToDic(withJsonString: evParams) as? [String: Any],
                    ...
                    mindMazeSendEvents(withParams: evParams)
                }
            }
        }
    }
